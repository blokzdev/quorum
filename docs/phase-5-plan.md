...
> world-class implementation"*). Phase 5 pulls the **Edge Model Draft Board** forward from the post-V1
...
| **Core** | 12–24GB | `qwen3.5:9b` (6.6GB) | `qwen3.5:4b` (3.4GB) for slower machines; `gemma4:e2b` (7.2GB, thinking) alternate; `qwen3:14b` (9.3GB, prev-gen) upper option |
| **Pro** | ≥ 24GB | `qwen3.6:35b` (35B-A3B MoE, 24GB, ~3B active) | fallback `qwen3.5:27b` dense (17GB) if the reported Ollama MoE GPU-utilization issue bites; `gemma4:e4b`/`12b` alternates |

Catalog entries carry: exact Ollama tag, byte size, min-RAM, capability (analyst-capable vs text-only),
license, and a one-line "why this one". **Standardizing on one family per tier** means one download
serves analysts *and* debaters.
...
- [ ] **P5.1a Catalog data + contract** — the curated catalog as versioned data served by the sidecar
  ...
  role-capability, license, blurb. Engine-side so a future hosted/curated update path (open-core) stays
  possible; ships frozen defaults. Re-verify every seed tag/size at implementation time.
- [ ] **P5.1b Device capability detection** — total RAM via `device_info_plus` on Windows (VRAM
  refinement is a backlog line, not V1); pure tier-assignment function (RAM → Lite/Core/Pro), unit-tested.
- [ ] **P5.1c Fit badges** — per-model **Fits / Tight / Won't-fit** from exact model bytes + the KV-cache
  formula at the engine's configured context, pure Dart, unit-tested against the live-verified llama3.2
  worked example (1.9GB file + 0.44GiB KV @4K ctx).
...
- [ ] **P5.2a Pull seam** — sidecar-proxied pull (`POST /pull` → Ollama `/api/pull`, streamed over the
  existing SSE/job machinery — seam decision validated at recon against the run-job seam; default is
  sidecar-proxied for consistency with `/catalog/local-models`).
...
- [ ] **P5.2c Post-pull integration** — a completed pull folds into the existing discovery + capability
  gate **without app restart**; pull errors (Ollama down, disk full, cancelled) surface honestly.
  *Exit (falsifiable):* a real curated model pulls end-to-end from the Draft Board on the dev machine;
  ...
  its true capabilities; killing Ollama mid-pull produces a recoverable error state (not a crash);
  goldens for pulling/installed/error states.

### P5.3 — Tiered free lineup + zero-key onboarding *(the payoff)*

- [ ] **P5.3a Preset local Benches** — one-click **"Free local team"** presets per tier
  (Starter/Balanced/Max mapped to Lite/Core/Pro), built on the existing Bench + roster mechanics; the
  preset only offers models that are installed (or routes through P5.2 pull affordances).
...
- [ ] **P5.3c Zero-key onboarding** — a keyless first-run Hub affordance surfacing the free-local path
  (detect: no provider keys + Ollama present/absent → guide to the Draft Board); honest copy (local
  models are slower/less capable than frontier cloud models — no over-promising). Golden-tested.
  *Exit (falsifiable):* on a keyless config, the Hub visibly offers the free-local path (golden); a
  tier preset applies a complete valid roster in one click; the roster-fit function is unit-tested for
  the max-not-sum rule; **a full real analysis run completes on a roster composed entirely of curated
  local models on the dev machine** (the end-to-end proof).

### P5.4 — Real-run verification sweep + close-out *(the honesty gate)*

- [ ] **P5.4a Analyst-default verification** — every curated **analyst-default** model (Lite + Core
  minimum) passes a **real gated run**: tool_calls actually fire and a non-empty report is produced
  through the live capability gate. A model that fails is **demoted to text-only in the catalog data**
  (that demotion path existing is itself an exit criterion). Pro-tier (24GB) verified if dev hardware
  allows; otherwise its entries carry an explicit `verified: tag-only` marker in the catalog — **no
  silent unverified defaults.**
